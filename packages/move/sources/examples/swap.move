...
module legato_addr::swap {

    use aptos_framework::coin::{Self, Coin, MintCapability, BurnCapability};
    use std::string::{Self, String};
    use std::option; 
    use std::signer::address_of;
    use aptos_std::math128::{Self, mul_div, min, sqrt};
    ...
    public entry fun create_pool<X, Y>(sender: &signer) {
        ...
        assert!(!pair_exist<X, Y>(@legato_addr), 1000);
    ...
    public entry fun add_liquidity<X, Y>(sender: &signer, x_amount: u64, y_amount: u64) acquires Pair {
        ...
        assert!(exists<Pair<X, Y>>(@legato_addr), 1000);

        // Borrow the Pair data from global storage
        let pair = borrow_global_mut<Pair<X, Y>>(@legato_addr);
        ...
        if (total_supply == 0){
            liquidity = ( sqrt(((x_amount * y_amount) as u128)) - (MINIMUM_LIQUIDITY as u128) as u64);
            let lp_locked = coin::mint(MINIMUM_LIQUIDITY, &pair.lp_mint);
            coin::merge(&mut pair.lp_locked, lp_locked);
        } else {
            liquidity = (min(
                math128::mul_div(x_amount, total_supply, x_reserve),
                math128::mul_div(y_amount, total_supply, y_reserve),
            ) as u64);
        };
    ...
    public entry fun remove_liquidity<X, Y>(sender: &signer, liquidity: u64) acquires Pair {
        ...
        let x_amount = math128::mul_div((liquidity as u128), x_reserve, total_supply);
        let y_amount = math128::mul_div((liquidity as u128), y_reserve, total_supply);
    ...
    public fun quote(x_amount:u128,x_reserve:u128,y_reserve:u128):u128{
        math128::mul_div(x_amount,y_reserve,x_reserve)
    }
    ...
    public fun get_coin<X,Y>():(u64,u64) acquires Pair {
        let pair = borrow_global<Pair<X,Y>>(@legato_addr);
